...
class WakeWordTrainer:
    ...
    def create_model(arch_name: str, featurizer: str, feature_dim: int = None,
        ...
        if shared_extractor is not None:
            ...
            if featurizer_type == "onnx":
                extractor = OnnxFeatureExtractor(featurizer, sample_rate, device)
            elif featurizer_type == "mfcc":
                extractor = MfccExtractor(sr=sample_rate)
            elif featurizer_type == "filterbank":
                n_mels = kwargs.get("n_mels", 80)
                extractor = FilterbankExtractor(sr=sample_rate, n_mels=n_mels)
            elif featurizer_type == "sincnet":
                n_filters = kwargs.get("n_filters", 80)
                extractor = SincNetExtractor(sr=sample_rate, n_filters=n_filters)
            elif featurizer_type in ("hubert",):
                extractor = HubertExtractor(featurizer, sample_rate, device)
            elif featurizer_type in ("wav2vec2",):
                extractor = Wav2Vec2Extractor(featurizer, sample_rate, device)
            elif featurizer_type in ("wav2vec2bert",):
                extractor = Wav2Vec2BertExtractor(featurizer or "facebook/w2v-bert-2.0",
                                                  sample_rate, device)
            elif featurizer_type == "delta_mfcc":
                base = MfccExtractor(sr=sample_rate, n_mfcc=kwargs.get("n_mfcc", 40))
                extractor = DeltaExtractor(base)
            elif featurizer_type == "gammatone":
                n_filters = kwargs.get("n_filters", 64)
                extractor = GammatoneExtractor(sr=sample_rate, n_filters=n_filters)
            elif featurizer_type == "delta_filterbank":
                n_mels = kwargs.get("n_mels", 80)
                base = FilterbankExtractor(sr=sample_rate, n_mels=n_mels)
                extractor = DeltaExtractor(base)
            else:
                raise ValueError(f"Unknown featurizer_type: {featurizer_type!r}. "
                                 f"Choose from: onnx, mfcc, filterbank, sincnet, "
                                 f"hubert, wav2vec2, wav2vec2bert, delta_mfcc, gammatone, delta_filterbank")

        if feature_dim is None:
            feature_dim = extractor.feature_dim

        if arch_name == "gru":
            cls = GruClassifierHead
            args = {"hidden_dim", "dropout", "bidirectional", "gru_n_layers"}
        elif arch_name == "cnn":
            cls = CnnClassifierHead
            args = {"conv_dim", "linear_dim", "kernel_size", "stride"}
        elif arch_name == "ffn":
            cls = FfnClassifierHead
            args = {"hidden_dim", "dropout"}
        else:
            raise ValueError(f"Unknown classifier architecture: {arch_name!r}")

        inst_kwargs = {k: v for k, v in kwargs.items() if k in args}
